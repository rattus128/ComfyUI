...
class ModelPatcher:
    ...
    def load(self, device_to=None, lowvram_model_memory=0, force_patch_weights=False, full_load=False):
        with self.use_ejected():
            ...
            for x in loading:
                ...
                lowvram_fits = mem_counter + module_mem + potential_offload < lowvram_model_memory or \
                               mem_counter + module_mem < comfy.model_management.MINIMUM_MODEL_MEMORY
    ...
    def partially_unload(self, device_to, memory_to_free=0, force_patch_weights=False, allow_full_unload=True):
        with self.use_ejected():
            ...
            for unload in unload_list:
                if memory_to_free + offload_buffer - self.model.model_offload_buffer_memory < memory_freed:
                    break
                if (not allow_full_unload) and self.model.model_loaded_weight_memory - comfy.model_management.MINIMUM_MODEL_MEMORY < memory_freed:
                    break
    ...
    def partially_load(self, device_to, extra_memory=0, force_patch_weights=False):
        with self.use_ejected(skip_and_inject_on_exit_only=True):
            ...
            if extra_memory < 0 and not unpatch_weights:
                self.partially_unload(self.offload_device, -extra_memory, force_patch_weights=force_patch_weights, allow_full_unload=False)
                return 0
            full_load = False
