...
class HookPatch:
    ...
    def __call__(self, weight, original_weight):
        original_weights = {}
        #The bare minimum that keeps model-as-lora implementation happy
        original_weights[self.key] = [(weight)]
        #FIXME: see if we can dump this float32 cast up
        temp_weight = comfy.lora.calculate_weight(self.patches[self.key], temp_weight, self.key, intermediate_dtype=torch.float32, original_weights=original_weights)
...
class ModelPatcher:
    ...
    def get_comfy_module_and_param_name(self, key):
        op_keys = k.rsplit('.', 1)
        ...
        try:
            ...
            return None, None
        if not op or not hasattr(op, "comfy_cast_weights"):
            return None, None
        return op, op_keys[1]
    ...
    def state_dict_for_saving(self, clip_state_dict=None, vae_state_dict=None, clip_vision_state_dict=None):
        ...
        for k, v in unet_state_dict.items():
            op, param = get_comfy_module_and_param_name(k)
            if op is None or (hasattr(op, "comfy_patched_weights") and op.comfy_patched_weights == True):
                continue
...
class ModelPatcherDynamic(ModelPatcher):
    ...
    def unpatch_model(self, device_to=None, unpatch_weights=True):
        ...
        #NOTE: Pinning always happens on the unpatched weights, so theres no patching reason
        #to unpin
        if unpatch_weights:
    ...
    def partially_load(self, device_to, extra_memory=0, force_patch_weights=False):
        ...
        with self.use_ejected(skip_and_inject_on_exit_only=True):
            ...
            #FIXME: The API for full loading with 1e32 is a bit broken
            try:
    ...
    def patch_hook_weight_to_device(self, hooks: comfy.hooks.HookGroup, combined_patches: dict, key: str, original_weights: dict, memory_counter: MemoryCounter):
        ...
        op, param = get_comfy_module_and_param_name(key)
        if op is not None:
            setattr(op, param + "_hooks", HookPatch(key, combined_patches))
            op._v_signature = None
            if key not in self.hook_backup:
                self.hook_backup[key] = (op, param +"_hooks")
            return

        weight, set_func, convert_func = get_key_weight(self.model, key)
        weight: torch.Tensor

        weight.comfy_hook = HookPatch(key, combined_patches)
        weight._v_signature = True
        if key not in self.hook_backup:
            self.hook_backup[key] = (op, "comfy_hook")
    ...
    def unpatch_hooks(self, whitelist_keys_set: set[str]=None) -> None:
        with self.use_ejected():
            ...
            keys = list(self.hook_backup.keys())
            for k in keys:
                if not whitelist_keys_set or k in whitelist_keys_set:
                    item, extra_arg = self.hook_backup[k]
                    delattr(item, extra_arg)    
                    item._v_signature = None
                    self.hook_backup.pop(k)
                self.current_hooks = None
